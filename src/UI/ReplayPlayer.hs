...
import Control.Concurrent (MVar, forkIO, modifyMVar_, readMVar, threadDelay, newMVar)
...
import Linear.V2 (V2 (..))
import Logging.Replay (ReplayState (..), handleSpeed)
...
replayEventHandler (VtyEvent (V.EvKey k _)) _ mSpeedMod = gets rEvIndex >>= \ix -> liftIO $ do
  case k of
    V.KChar 'n' -> modifyMVar_ mSpeedMod (pure . normalSpeed)
    V.KChar 'r' -> modifyMVar_ mSpeedMod (pure . negate) >>  print ix
...
initState seed = ReplayState {
    rTickNo = TickNumber 0
  , rGameState = start
  , rGameStateVec = Right []
  , rCheckPoint = Map.empty
  , rEvIndex = 0
  , rRewindIndex = 0
  }
  where
...
drawUI rps = [drawDebug rps] <> (center <$> drawGS gs)
  where
...
drawDebug :: ReplayState -> Widget n
drawDebug gps = currentTick
  where
    currentTick = hLimit 10 $ vBox [txt $ Text.pack $ show $ (rTickNo gps)]
...
mkEvs = reverse . filter (\(GameEvent _ x) -> x `elem` [MoveUp, MoveDown, MoveLeft, MoveRight])
