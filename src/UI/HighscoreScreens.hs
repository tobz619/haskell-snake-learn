...
inputHandler (AppEvent Tick) = do
  ...
  !heartbeat <- if not onl then pure True else liftIO . readMVar =<< use connection 
  let !connectionLost = foldr1 (&&) [onl, heartbeat, heartbeat == False]
  if connectionLost
    then do
