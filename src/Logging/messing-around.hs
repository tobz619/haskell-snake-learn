...
instance Show Board where
  show ( MkBoard d cs ) = let
    remDeck = "Deck size: " ++ (show (length d))
    showColumns = labelCols cs
    in unlines [remDeck,"", showColumns] 

nums = [101..152]

mkBoard = go [1..7] []
  where go [] acc rest = MkBoard rest (reverse acc)
        go (n:ns) cols ds = let (col,rest) = ((\x -> (x , False)) <$> take n ds, drop n ds)
